...
.MainGameLoop:
; main game loop
    lda EnemySpawn              ; load EnemySpawn flag
    cmp #0                       ; compare to #1
    beq .RelicLoop              ; if flag == 1, go to the ghost loop (ghost is spawned)
; Game loop for ghost mode
.GhostLoop:
;
    jsr AreWeInsideChar        ; subroutine for drawing the character sprite
.IsGhostSpawned:
    lda Spawn
    cmp #1
    bne .GhostRenderRemaining
    lda #0
.AreWeInsideGhostSprite:
    ...
    bcc .DrawSpriteGhost           ; if true, call draw routine
...
.GhostRenderRemaining:         ; if there isnt something spawned,
    lda Spawn                   ; we need to render the rest of the screen
    cmp #1                       ; compare spawn flag to #1
    beq .EndRender              ; if spawn != 1, jump to endRender
    sta WSYNC                   ; draw line
    jmp .EndRender              ; jump to endRender anyway
...
.IsSpawned:                     ; check if relic has spawned
    lda Spawn                   ; load Spawn flag
    cmp #1                      ; is spawn == 1? (is relic spawned?)
    bne .RenderRemaining       ; if no, then render remaining lines
    lda #0                      ; else load 0 in A and proceed to render relic
...
.EndRender:
    dex                         ; X--
    bne .MainGameLoop          ; repeat until 192 lines are done
...
UpdateGhost:
    ...
    REPEAT 6                     ; repeat it 7 times (to make the ghost faster)
